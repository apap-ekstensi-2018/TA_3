...
public class SuratServiceDatabase implements SuratService {
	...
	public List<PengajuanSuratModel> selectAllPengajuanSurat() {
		...
		for (int i = 0; i < pengajuanSuratModels.size(); i++) {
			JenisSuratModel jenisSuratModel = suratMapper.selectJenisSuratById(
					((ArrayList<PengajuanSuratModel>) pengajuanSuratModels).get(i).getId_jenis_surat());
			pengajuanSuratModels.get(i).setJenis_surat(jenisSuratModel);
			StatusSuratModel statusSuratModel = suratMapper
					.selectStatusSuratById(pengajuanSuratModels.get(i).getId_status_surat());
	...
	@Override
	public List<PengajuanSuratModel> selectPengajuanSuratByTanggalSurat(Date tanggal_awal_mohon,
			Date tanggal_akhir_mohon) {
		log.info("Tampilkan data pengajuan surat");
		List<PengajuanSuratModel> pengajuanSuratModels = suratMapper
				.selectPengajuanSuratByTanggalSurat(tanggal_akhir_mohon, tanggal_akhir_mohon);
		for (int i = 0; i < pengajuanSuratModels.size(); i++) {
			JenisSuratModel jenisSuratModel = suratMapper.selectJenisSuratById(
					((ArrayList<PengajuanSuratModel>) pengajuanSuratModels).get(i).getId_jenis_surat());
			pengajuanSuratModels.get(i).setJenis_surat(jenisSuratModel);
			...
			// suratMapper.selectStatusSuratById(pengajuanSuratModels.get(i).getId_status_surat());
			// pengajuanSuratModels.get(i).setUsername_pengaju(null);;
		}
		return pengajuanSuratModels;
	}
